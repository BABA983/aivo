...
async fn test_ask_card_multi_select_space_toggles_and_enter_joins() {
    ...
    assert!(
        ...
        "space toggles without confirming"
    );
    app.handle_key(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE))
        .await
        .unwrap();
...
fn test_review_card_renders_diff_and_keys() {
    ...
    app.cards.set_review(PendingReview {
        count: 1,
        body,
        scroll: 0,
        reply,
...
async fn test_review_card_arrows_scroll() {
    ...
    app.handle_key(KeyEvent::new(KeyCode::Down, KeyModifiers::NONE))
        .await
        .unwrap();
    assert_eq!(app.cards.review().unwrap().scroll, 2, "down scrolls");
    app.handle_key(KeyEvent::new(KeyCode::Up, KeyModifiers::NONE))
        .await
        .unwrap();
    assert_eq!(app.cards.review().unwrap().scroll, 1, "up scrolls back");
...
async fn test_review_card_overscroll_clamps_stable_height() {
    ...
    let clamped = app.cards.review().unwrap().scroll;
    assert!(
