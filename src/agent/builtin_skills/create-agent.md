...
- Built-in: `explorer` (read-only exploration), `aivo-guide` (aivo docs expert), `verification` (adversarial PASS/FAIL checks), `advisor` (read-only second opinion), and `evaluate` (code review) ship inside aivo, at the lowest precedence — creating a same-named file replaces them, and that's the supported way to customize one (start from the built-in's text, shown by `/agents` inside `aivo code`); warn before authoring a new agent under one of these names, since it silently shadows the built-in
